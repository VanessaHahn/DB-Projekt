...
module.exports = (function () {
  ...
  function addDefaultValuesIntoDatabase(connection) {
    ...
    drivers.count({}, function (err, count) {
      //Prevent the default values to be inserted multiple times
      if(count === 0){
        addDriver({
          "name": "John Smith",
          "passwort": "john-smith",
  ...
  function addAdress(adress) {
    return new Promise(function (resolve, reject) {
      adresses.findOne({avenue: adress.avenue, street: adress.street}, function (err, adressExists) {
        if (adressExists === null) {
          adresses.create(adress, function (error, newAddress) {
            if (error) {
              reject(error);
            } else {
              resolve(newAddress);
